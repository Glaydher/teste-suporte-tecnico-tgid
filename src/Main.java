...
public class Main {
	...
	public static void executar(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
		...
		Usuario userLog = userAuthentication(usuarios, username, senha);

		if (userLog != null) {
			System.out.println("Escolha uma opção para iniciar");
			if (userLog.isEmpresa()) {
				while(true){
					companyMenu();
					int escolha = sc.nextInt();
					switch (escolha) {
						case 1:
							companySales(userLog, vendas);
							break;
						case 2:
							showProducts(userLog, produtos);
							break;
						case 0:
							executar(usuarios, clientes, empresas, produtos, carrinho, vendas);
						default:
				...
				else if (userLog.isCliente()) {
					while(true){
						clientMenu();
						int escolha = sc.nextInt();
						switch (escolha) {
							case 1:
								buy(userLog, empresas, produtos, carrinho, vendas, clientes);
								break;
							case 2:
								shop(userLog, vendas);
								break;
	...
	private static Usuario userAuthentication(List<Usuario> usuarios, String username, String senha) {
	...
	private static void companySales(Usuario empresa, List<Venda> vendas) {
		System.out.println();
		System.out.println("************************************************************");
		System.out.println("VENDAS EFETUADAS");
		final double[] taxes = { 0.00 };
		vendas.stream().forEach(venda -> {
			if (venda.getEmpresa().getId().equals(empresa.getEmpresa().getId())) {
				System.out.println("************************************************************");
				...
				System.out.println("************************************************************");
				taxes[0] += venda.getComissaoSistema();
			}
		});
		System.out.println("Saldo Empresa: " + (empresa.getEmpresa().getSaldo() - taxes[0]));
		System.out.println("************************************************************");
	}

	private static void showProducts(Usuario empresa, List<Produto> produtos) {
		System.out.println();
		System.out.println("************************************************************");
		System.out.println("MEUS PRODUTOS");
		produtos.stream().forEach(produto -> {
			if (produto.getEmpresa().getId().equals(empresa.getEmpresa().getId())) {
				System.out.println("************************************************************");
		...
		System.out.println("Saldo Empresa: " + empresa.getEmpresa().getSaldo());
		System.out.println("************************************************************");
	}

	private static void buy(Usuario cliente, List<Empresa> empresas, List<Produto> produtos,
		...
		do {
			...
			for (Produto produtoSearch : produtos) {
				if (produtoSearch.getId().equals(escolhaProduto) && escolhaEmpresa == produtoSearch.getEmpresa().getId())
		...
		Cliente clienteLogado = clientes.stream()
				.filter(x -> x.getUsername().equals(cliente.getUsername()))
				.collect(Collectors.toList()).get(0);
		Venda venda = createSale(carrinho, empresaEscolhida, clienteLogado, vendas);
	...
	private static void shop(Usuario cliente, List<Venda> vendas) {
		System.out.println();
		System.out.println("************************************************************");
	...
	public static Venda createSale(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
	...
	private static void showCompanies(List<Empresa> empresas) {
		Scanner sc = new Scanner(System.in);
